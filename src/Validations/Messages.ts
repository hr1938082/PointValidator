...
const Messages = (key: string, ruleKey: Keys, ruleKeyParams?: string[]): string => {
    const messages: Record<Keys, string> = {
        ...
        must_contains_numeric: `${key} must contains number`,
        must_contains_small_case: `${key} must contains small case`,
        must_contains_symbols: `${key} must contains symbols`,
        must_contains_upper_case: `${key} must contains upper case`,
